...
class AJAInput : public Node {
  GDCLASS(AJAInput, Node)
...
public:
  ...
  bool open(int p_device, int p_channel,
            int64_t p_video_format = NTV2_FORMAT_UNKNOWN);
  void close();
  bool is_open() const;
...
private:
  void _capture_thread_loop();
  void _start_capture_thread();
  void _stop_capture_thread();
  ...
  int _device = 0;
  int _channel = 0;
  NTV2VideoFormat _video_format = NTV2_FORMAT_UNKNOWN;
